...
fun RommCollectionListScreen(
    sections: List<ListSection<RommCollection>>,
    selectedIndex: Int,
    scrollTarget: Int,
    ...
    val listState = rememberLazyListState(initialFirstVisibleItemIndex = scrollTarget.coerceAtLeast(0))
    ...
    ScreenBackground(backgroundImagePath = backgroundImagePath, backgroundTint = backgroundTint) {
        Box(modifier = Modifier.fillMaxSize().padding(screenPadding)) {
            Column(modifier = Modifier.fillMaxSize().padding(bottom = footerReservation())) {
                ...
                SectionedList(
                    sections = sections,
                    selectedIndex = selectedIndex.coerceIn(0, (allItems.size - 1).coerceAtLeast(0)),
                    fontSize = listFontSize,
                    lineHeight = listLineHeight,
